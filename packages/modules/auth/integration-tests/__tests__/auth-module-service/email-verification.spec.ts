...
moduleIntegrationTestRunner<IAuthModuleService>({
  ...
  testSuite: ({ MikroOrmWrapper, service }) => {
    describe("AuthModuleService - email verification tokens", () => {
      beforeEach(async () => {
      ...
      it("generates an opaque token and stores only its hash", async () => {
        const result = await service.requestAuthEmailVerification({
          actor_type: "user",
          provider: "emailpass",
        ...
        expect(result).toEqual({
          ...
          email_verification: expect.objectContaining({
            actor_type: "user",
            provider: "emailpass",
        ...
        expect(result.email_verification).not.toHaveProperty("token")

        const [storedToken] = await MikroOrmWrapper.forkManager().execute(
          "select * from auth_email_verification_token where provider_identity_id = ?",
          ["provider-id"]
        )
      ...
      it("invalidates existing unused tokens when requesting a new token", async () => {
        const first = await service.requestAuthEmailVerification({
          provider: "emailpass",
          email: "verify@test.com",
        })
        const second = await service.requestAuthEmailVerification({
          provider: "emailpass",
          email: "verify@test.com",
        })
        ...
        await expect(
          service.confirmAuthEmailVerification({ token: first.token })
        ).rejects.toThrow("Email verification token is invalid or already used")
      })

      it("confirms a token, consumes it, and marks the provider identity verified", async () => {
        const { token } = await service.requestAuthEmailVerification({
          provider: "emailpass",
          email: "verify@test.com",
        })

        await expect(
          service.confirmAuthEmailVerification({ token })
        ).resolves.toEqual({
          email_verified: true,
          auth_identity_id: "auth-id",
          provider_identity_id: "provider-id",
        ...
        const [storedToken] = await MikroOrmWrapper.forkManager().execute(
          "select used_at from auth_email_verification_token where provider_identity_id = ?",
          ["provider-id"]
        )

        expect(
          parseJson(providerIdentity.provider_metadata).email_verified_at
      ...
      it("rejects expired, used, and unknown tokens", async () => {
        const expired = await service.createAuthEmailVerificationToken({
          auth_identity_id: "auth-id",
          provider_identity_id: "provider-id",
          email: "verify@test.com",
        ...
        await expect(
          service.confirmAuthEmailVerification({ token: expired.token })
        ).rejects.toThrow("Email verification token has expired")

        const active = await service.requestAuthEmailVerification({
          provider: "emailpass",
          email: "verify@test.com",
        })

        await service.confirmAuthEmailVerification({ token: active.token })
        await expect(
          service.confirmAuthEmailVerification({ token: active.token })
        ).rejects.toThrow("Email verification token is invalid or already used")
        await expect(
          service.confirmAuthEmailVerification({ token: "missing" })
        ).rejects.toThrow("Email verification token is invalid or already used")
      })
    })
  },
...
moduleIntegrationTestRunner<IAuthModuleService>({
  ...
  testSuite: ({ service }) => {
    describe("AuthModuleService - email verification authentication flow", () => {
      beforeEach(async () => {
        jest.spyOn(Date, "now").mockReturnValue(1_710_000_000_000)
        await createUnverifiedIdentity(service)
      })
      ...
      it("returns email verification required before MFA challenge creation", async () => {
      ...
      it("continues to MFA once the emailpass identity is verified", async () => {
        ...
        const verification = await service.requestAuthEmailVerification({
          provider: "emailpass",
          email: "verify@test.com",
        })
        ...
        await service.confirmAuthEmailVerification({
        ...
        expect(result.email_verification_required).toBeUndefined()
        expect(result.authIdentity).toBeUndefined()
      })
    })
  },
