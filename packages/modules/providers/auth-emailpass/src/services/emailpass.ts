...
type EmailPassProviderMetadata = {
  password?: unknown
...
export class EmailPassAuthService extends AbstractAuthModuleProvider {
  ...
  private async upsertAuthIdentity(
    ...
    const passwordHash = await this.hashPassword(password)
    const providerMetadata: EmailPassProviderMetadata = {
      password: passwordHash,
    }

    if (this.requiresEmailVerification_()) {
      providerMetadata.email_verified_at = null
    }
  ...
  private requiresEmailVerification_(): boolean {
    return this.config_.require_email_verification === true
  }

  private async getProviderMetadata_(
    entityId: string,
    authIdentityService: AuthIdentityProviderService
  ): Promise<EmailPassProviderMetadata> {
  ...
  private sanitizeAuthIdentity_(authIdentity: AuthIdentityDTO): AuthIdentityDTO {
